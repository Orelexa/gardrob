...
import { RotateCcwIcon, ChevronLeftIcon, ChevronRightIcon } from './icons.tsx';
  ...
  return (
    <div className="w-full h-full flex items-center justify-center relative animate-zoom-in group">
      ...
      <div className="relative w-full h-full flex items-center justify-center">
        {displayImageUrl ? (
          <SmartImage
            key={displayImageUrl}         // képváltásnál animáció miatt
            src={displayImageUrl}
            alt="Virtuális próba modell"
            // adunk minimális helyfoglalást, hogy ne ugorjon a layout
            width={400}
            height={600}
            sizes="(max-width: 768px) 90vw, 70vw"
            className="max-w-full max-h-full object-contain transition-opacity duration-500 animate-fade-in rounded-lg"
            // nagyon kicsi, elmosott placeholder (szükség esetén cseréld valódi LQIP-re)
            placeholderSrc="data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iNDAwIiBoZWlnaHQ9IjYwMCIgZmlsbD0iI2VlZWVlZSIgeG1sbnM9Imh0dHA6Ly93d3cudzMub3JnLzIwMDAvc3ZnIi8+"
            lazy
          />
        ) : (
          <div className="w-[400px] h-[600px] bg-gray-100 border border-gray-200 rounded-lg flex flex-col items-center justify-center">
            <Spinner />
            <p className="text-md font-serif text-gray-600 mt-4">Modell betöltése...</p>
          </div>
        )}
