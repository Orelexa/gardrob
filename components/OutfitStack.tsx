    ...
    <div className="flex flex-col">
      ...
      <div className="space-y-2">
          ...
          >
            <div className="flex items-center overflow-hidden">
                ...
                <span className="font-semibold text-gray-800 truncate" title={layer.garment?.name}>
                  {layer.garment ? layer.garment.name : 'Alap modell'}
                </span>
            </div>
            {index > 0 && index === outfitHistory.length - 1 && (
               <button
                ...
                aria-label={`${layer.garment?.name} eltávolítása`}
              >
        ...
        {outfitHistory.length === 1 && (
            <p className="text-center text-sm text-gray-500 pt-4">Az összeállított ruhadarabok itt fognak megjelenni. Válassz egyet a gardróbból.</p>
        )}
      </div>
    </div>
  );
